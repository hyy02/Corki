...
class BaseCalvinDataset(Dataset):
    ...
    def process_waypoint(
        ...
        seq_waypoint = torch.from_numpy(episode["is_waypoint"]).float()
        return {"is_waypoint": seq_waypoint}
    ...
    def _get_sequences(self, idx: int, window_size: int, head: bool = False) -> Dict:
        ...
        if self.waypoint:
            seq_waypoint = self.process_waypoint(episode)
            seq_dict['is_waypoint'] = seq_waypoint['is_waypoint']
...
class DiskCalvinDataset(BaseCalvinDataset):
    ...
    def __init__(
        ...
        # if self.waypoint:
            ...
            #     self.waypoint_dict = json.load(file)
        ...
        # self.file_path = {}
        # for tmp in os.listdir(self.abs_datasets_dir):
        #     tmp = os.path.join(self.abs_datasets_dir, tmp)
        #     if os.path.isdir(tmp):
        #         for tmp2 in os.listdir(tmp):
        #             tmp2 = os.path.join(tmp, tmp2)
        #             self.file_path[os.path.basename(tmp2)] = tmp2
        #     elif self.save_format in tmp:
        #         self.file_path[os.path.basename(tmp)] = tmp
        # for f in list(self.file_path.values()):
        #     if self.save_format in f and self.lang_folder not in f:
        #         filename = Path(f)
        #         Disfilename = Path(f)
        #         break
        ...
        # assert n_digits > 0
        # self.naming_pattern, self.n_digits = naming_pattern, n_digits
    ...
    def _get_episode_name(self, file_idx: int) -> Path:
        ...
        #                 f"{os.path.basename(self.naming_pattern[0])}{file_idx:0{self.n_digits}d}{self.naming_pattern[1]}"])
    ...
    def _load_episode(self, idx: int, window_size: int) -> Dict[str, np.ndarray]:
        ...
        if self.waypoint:
            is_waypoint = np.zeros(end_idx - start_idx, dtype=np.bool_)
            ith = 0
            is_waypoint[ith] = True  # if np.random.random() > 0.5 else False # 第一个不一定是关键点
            while ith <= window_size - self.act_step:
                trajectories = np.array([np.concatenate(
                    [data[self.observation_space['actions'][0]][:3], data[self.observation_space['actions'][0]][3:6],
                     (data[self.observation_space['actions'][0]][[-1]] > 0).astype(np.float32)], axis=-1)
                                         for data in episodes[ith:ith + self.act_step]])
                trajectories[..., :6] = np.cumsum(trajectories[..., :6], axis=0)
                jth = 1
                local_max_A = []  # 注意这里边只有一个
                p_st = trajectories[0][:3]
                while jth < len(trajectories):
                    p_ed = trajectories[jth][:3]
                    distance_max = 0
                    for kth in range(1, jth):
                        p = trajectories[kth][:3]
                        if np.degrees(np.arccos(np.dot(p_ed - p, p_ed - p_st) / (
                            ...
                            distance = np.sin(np.arccos(np.dot(p - p_st, p_ed - p_st) / (
                                        np.linalg.norm(p - p_st) * np.linalg.norm(p_ed - p_st)))) * np.linalg.norm(
                                p - p_st)
                        distance_max = max(distance_max, distance)
                    # print(distance_max)
                    if distance_max > self.threshold:  # 0.03:
        ...
        if self.waypoint:
            # waypoint_list = [
            #     np.array(self.waypoint_dict[str(file_idx)])
            #     for file_idx in range(start_idx, end_idx)
            # ]
            # waypoint_np = np.stack(waypoint_list)
            episode["is_waypoint"] = is_waypoint
        return episode
    ...
    def _build_file_indices_lang(
        ...
        for i, (start_idx, end_idx) in enumerate(ep_start_end_ids):
            ...
            for idx in range(start_idx, end_idx + 1 - self.min_window_size):
                # for idx in range(start_idx, end_idx + 1 - self.min_window_size * self.skip_steps + self.act_step): #调增窗口位置
    ...
    def collater(self, sample):
        # if self.waypoint:
        #     action_tensors = torch.from_numpy(np.array([np.stack(s["waypoint"]) for s in sample]))
        ...
        if self.waypoint:
            is_waypoint = torch.from_numpy(np.array([np.stack(s["is_waypoint"]) for s in sample]))
        ...
        # if self.waypoint:
        #     max_wp_idx = is_waypoint.max() + 1
        #     image_tensors = image_tensors[:, :max_wp_idx]
        #     action_tensors = action_tensors[:, :max_wp_idx]
        #     gripper_tensors = gripper_tensors[:, :max_wp_idx]
        #     state_tensors = state_tensors[:, :max_wp_idx]
        #     robot_obs = robot_obs[:, :max_wp_idx]
        if self.waypoint:
            return image_tensors, (text_tensors, attention_mask), action_tensors, gripper_tensors, state_tensors, robot_obs,is_waypoint
        else:
...
class CalvinSim(Dataset):
    def __init__(self,
        ...
        # dataset_path = "/mnt/dolphinfs/hdd_pool/docker/user/hadoop-vacv/yanfeng/data/robotics/task_D_D"
...
def get_dataset_fn(dataset_type):
    if dataset_type == "calvin":
        ...
        return get_rlbench_dataset

    else:
        raise ValueError(f"Unsupported dataset type: {dataset_type}")


def get_data(args, image_processor, tokenizer, dataset_type, epoch=0,eval=False):
    if eval:
        return get_dataset_fn(dataset_type)(
            args, image_processor=image_processor, epoch=epoch, tokenizer=tokenizer,eval=eval)
    return get_dataset_fn(dataset_type)(
